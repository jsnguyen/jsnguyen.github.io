...
ra = "23 07 28.7157209544"
dec = "+21 08 03.310767492"
...
async def handle_click(event):
    ...
    print(ra_input.value)
    print(dec_input.value)
    ...
    print(lat_input.value)
    print(lon_input.value)
    print(height_input.value)
    location = EarthLocation(lon=float(lon_input.value), lat=float(lat_input.value), height=float(height_input.value)*u.meter)
    
    # Generate the plot
    fig = make_target_plot(location, ra_input.value, dec_input.value)
...
def make_target_plot(location, ra, dec):
    ...
    night_bound = find_bounds(night)[0]
    
    night_start = times[night_bound[0]]
    night_end = times[night_bound[1]]
    ...
    ax.text(0.5, 1.2, f'Time Visible {time_visible:.2f}\nMin Moon Sep: {np.min(moon_seps):.2f} deg',
